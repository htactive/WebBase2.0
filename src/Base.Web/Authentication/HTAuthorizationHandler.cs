...
namespace Base.Web.Authentication
{
    ...
    {
        private BaseDBRepository BaseDBRepository;
        ...
        {
            return BaseDBRepository.ClaimRepository.GetAll().Any(x => x.ClaimName == claim);
        }
        private void CreateNewClaim(string claim)
        {
            ...
            BaseDBRepository.ClaimRepository.Save(entity);
            BaseDBRepository.Commit();
        }

        private bool CheckClaims(string requireClaim, int userId)
        {
            return BaseDBRepository.UserRoleRepository.GetAll()
